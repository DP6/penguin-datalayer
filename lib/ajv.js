...
let validateObject = (schema, obj, filename, doc, exp_opt) => {
  let items = schema.array.items;
  let isSchemaEmpty = items.length === 0;
  let isObjEmpty = Object.entries(obj).length === 0 && obj.constructor === Object;

  let saveLog = (filename, status, message, dlObject, exp_opt) => {
    console.log(`${status}, ${message}, ${dlObject}\n`);
    // Export option as PDF
    if (exp_opt === 'pdf') {
      let logPDF = `${status} ${message} \n ${dlObject} \n\n`;
      fullPDF.push(logPDF);
      createPDF(fullPDF, filename, doc);
    } else {
      // Export option as XLSX
      fs.appendFileSync(filename, `${status}, ${message}, ${dlObject.split(',').join(' ')}\n`, (err) => {
        if (err) throw err;
      });
    }
  };
  ...
  let checkValidEvent = (items) => {
    for (let index = 0; index < items.length; index++) {
      ...
      if (valid) {
        saveLog(filename, 'OK', 'Validated Successfully', JSON.stringify(obj), exp_opt);
  ...
  let revalidateSchema = (shadowSchema, errorMessage, obj, schemaIndex, schemaArray, dlObj) => {
    ...
    if (verify_required == -1) {
      ...
      if (found > -1) {
        ...
        let isInnerSchemaEmpty =
          Object.entries(innerSchema.contains.properties).length === 0 && obj.constructor === Object; //um safe check pra garantir que o objeto não ficou vazio
        ...
        errors
          ...
          .map((eachError) => {
            ...
            if (isErrorDataEmpty) {
              ...
              revalidateSchema(shadowSchema, errorMessage, obj, index, arr, obj);
            }
          });
      }
    });
  ...
  let checkMissingEvents = (items, obj) => {
    ...
    missingEvents.map((event) => {
      saveLog(filename, 'ERROR', `Hit not validated or missed during test`, JSON.stringify(event), exp_opt);
    });
  };

  if (isSchemaEmpty) {
    saveLog(filename, 'ERROR', `No more items to validate`, JSON.stringify(obj), exp_opt);
